...
MaterialData(::C, consts::Tuple{Float64, Float64}) where {C <: Constant} = MaterialConstant{C}(consts)
(mc::MaterialConstant{C})(wavelength::Real) where {C <: Constant} = C(mc.data, ConstantIndex_N, wavelength)
...
MaterialData(::F, coeffs::NTuple{N,Float64}) where {N,F<:Formula} = MaterialFormula{N,F}(coeffs)
...
MaterialData(type::T, table::Matrix{Float64}) where {T<:Table} = MaterialTable(type, table)
